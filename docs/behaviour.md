...
| E7 | Dragging the BODY of a note moves it in time — one gesture handles both selecting and moving, even on an unselected note's very first click, with no separate select-then-drag step required. Dragging within ~8px of an EDGE resizes just that edge, but only once the note is already selected (its edge tabs need to be visible first — see E2/E24). Neither ever shrinks a note below a 20ms floor. Either drag recolours the note purple, same as E6. | Click-drag the middle of an UNSELECTED note in one continuous gesture: it becomes selected and both edges move equally, turning purple on release. Select a note first, then drag near its left edge: only `start` moves. Drag it past the floor: it stops at 20ms rather than crossing zero. Fill colour changes to purple in every case, including on a previously folded (blue) or doubt (gray) note. |
...
| E16 | With a note selected, ↑/↓ nudge pitch, Shift+↑/↓ shift octave, Delete/Backspace deletes, `G` snaps it to the grid (E38). ←/→ do NOT nudge a selected note's time — since the ergonomics batch (v1.16.1) they always seek the transport instead (see E20); time is adjusted via the toolbar's ◀t/▶t buttons or by dragging. | Select a note, press ↑: pitch moves. Press `G`: it snaps to the grid, same as clicking ⊞ (E38). Press →: the transport seeks (proportional to zoom span), the note's `start`/`end` are unchanged either way — selection makes no difference to ←/→. |
| E17 | **Export edits** — one shared button in the zoomed pane, beside the Edit-notes toggle, covering every analysed channel — downloads a single JSON file: `version: 3`, one shared `tempo`/`tempoRange`, and a `stems` object keyed by stem id, each entry holding `interpreter`, `params`, `clip`, `jianpu`, and `edits` (one `{edits, label?, timeLabel?}` object per list entry — `edits` has two elements for a split — so a batch's row label, e.g. "Snap to grid", survives the round trip instead of falling back to the generic multi-edit "Split" label). **Import edits**, the matching shared button, restores every control and the edit list for each stem the file and the currently loaded song both have, then re-derives each one's note list. See `lib/notes-edits.js` for the format and `docs/superpowers/specs/2026-08-31-note-editing-design.md`/the notes-shared-edit-io work for how it replaced the old per-panel pair. | Detect both channels, make an edit on each (including a range-snap batch), export, reload the same zip, re-run detection, import the file: `#notes-count-vocals`/`#notes-count-bass` and both channels' edit-list rows match, and the range-snap row still reads "Snap to grid" rather than "Split". |
| E42 | Every time value that reaches an edit (`at`/`dStart`/`dEnd`/`start`/`end`/`from`/`to`), a note's own `start`/`end`, or `tempoRange`'s `from`/`to` is rounded to millisecond precision (`lib/time.js`'s `roundSeconds`) before it is stored, dispatched, or exported — never the raw drag/arithmetic float (e.g. `207.45864999999998`). One shared function, called from `dispatchEdit` (app.js), `interpret()`/`applyEdits()` (lib/pitch.js), and `snapNoteEdges` (lib/ribbon.js), so no future call site can drift from it. | Drag-move a note by an arbitrary amount and export edits: every numeric time field in the downloaded JSON has at most 3 decimal digits. Same for a value read back from `#notes-edits-*`'s edit list or from `tempoRange` after dragging the drums-lane tempo-range band. |
| E34 | **Export list** (`#notes-list-export-vocals` / `#notes-list-export-bass`) downloads a self-contained HTML file (inline `<style>`, no external assets) of that channel's current 簡譜 reading laid out on the tempo grid — a separate, human-readable export from **Export edits**' JSON round-trip. Disabled **only** while there are zero notes (via `syncJianpuControls()`) — unlike the key selectors, it does NOT need 簡譜 itself to be on: `jianpu.tonic`/`jianpu.mode` always hold a real value, either the auto-detected key (once notes exist) or the C-major default before any override, so there's always a key to export against. The "Bars per line" field (`#notes-list-bars-vocals` / `#notes-list-bars-bass`, default 4) sets how many bars each line wraps at. Bar boundaries come from `lib/ribbon.js`'s `beatTimes()` against the shared tempo grid (the same grid the lane/zoomed-pane overlay draws from), padded with a leading pickup bar if playing notes start before the first detected bar and a trailing boundary at the song's end. Directly under the title, a `♩ = <bpm> <beatsPerBar>/4` line names the same shared tempo (`tempo.bpmValue` to one decimal, `tempo.beatsPerBar`) the "Show tempo grid" panel already reads from — the note value is always a quarter, matching `noteRhythm`'s fixed 4-units-per-beat grid. Each line is bracketed by a `\|` barline at both ends, with a `\|` between every bar in it too, so the wrap between one line's last bar and the next line's first is never mistaken for the absence of a barline; a full blank line separates one line from the next. A note whose span crosses a barline is split into per-bar fragments joined by a tie mark (⌣) — `lib/jianpu.js`'s `layoutBars()`. Each note's duration is quantised to a 16th-note grid and drawn as standard rhythm notation: 0/1/2 underlines for a quarter/eighth/sixteenth, one trailing dash per sustained extra beat, a trailing dot for a half-beat extension (`noteRhythm()`). Octave is drawn as real dots above (higher) or below (lower) the digit — standard 簡譜, stacked one per octave from the reference octave — not the apostrophe/comma punctuation the old Markdown version of this export used. Notes within a bar are left-aligned; bars in a line are equal-width regardless of how many notes each holds. Silent gaps between notes are left blank, no rest marks. Within a bar, fragments are always in start-time order — `layoutBars` sorts a copy before bucketing (same reason and same pattern as `lib/sonify.js`, see E23), so an `add`ed or split-off note lands at its correct position instead of at the end regardless of how late `applyEdits()` appended it to the note list. | With zero notes, `#notes-list-export-vocals.disabled` (or `-bass`) is `true`. Run **Find notes** without ever ticking 簡譜: it enables as soon as notes exist. Click it: the file downloads using the auto-detected key exactly as if 簡譜 had been ticked (see E35 for its exact filename and title). Open the downloaded file in a browser: every line is bracketed by `\|` at both ends with `\|` between bars, bars in a line render at equal width with left-aligned notes, a blank line visibly separates each line from the next, and a note held across a barline shows as two fragments joined by ⌣. A note one octave above the reference shows one dot above its digit; one below shows a dot underneath — never a `'` or `,` character. Change "Bars per line" from 4 to 2 and export again: each line now wraps after two bars instead of four, with the same bars/notes, just regrouped. With the tempo grid reading e.g. 206.9 BPM at 4 beats/bar, the exported file's second line reads `♩ = 206.9   4/4`; change "Beats/bar" to 3 and export again: it reads `3/4`. |
| E44 | **Export list** prints a bass-derived chord guess above each bar, from the bass channel's own notes/key (`lib/chords.js`'s `detectChords()`), regardless of which channel is being exported — vocals never drive a chord label, even when exporting the vocal list. Each bar is split at its time MIDPOINT into two halves; each half's label is the longest-overlap note's pitch class (root), with a diatonic triad quality (major/minor/dim, from the bass channel's own detected key) or a bare root name when that note is chromatic to the key, further relabelled to `sus4`/`sus2` when a 4th or major 2nd sounds SIMULTANEOUSLY with the root (temporally overlapping it) without the 3rd — `lib/pitch.js`'s segmenters (`segmentNotes`, `segmentNotesHmm`) always emit strictly sequential, non-overlapping notes from a monophonic bass line, so this overlap never occurs on notes from an ordinary **Find notes** run; in practice sus4/sus2 only appear once a note has been manually edited to overlap another. The bar's `.chords` row shows the first half's label top-left and the second half's label (only when it differs from the first) centered above the bar — `second` comes back `null` from `detectChords()` whenever that half is silent or matches the first half, so the caller never compares labels itself. With no bass stem loaded, or a bass stem never analysed, no `.chords` element is rendered on any bar, and the bar renders identically (visually) to before this feature (`chords` is `undefined`, not an array of nulls) — the underlying markup is not byte-identical, since `jianpuHtml` now always wraps a bar's note fragments in a `.frags` span regardless of whether chord data exists. | Detect notes on both channels of a song with a clear chord progression, export the vocal list: each bar shows a chord label top-left, and a bar where the bass line moves mid-bar (e.g. a walk from the root to its 4th) shows a second, centered label. Export the bass channel's own list: it is annotated with the SAME chords (derived from itself). Load a song with only a vocals stem (no bass), export: no chord row appears above any bar, and the bar's height/spacing visually matches a pre-chords export exactly. Load a bass stem but don't run **Find notes** on it, export the vocals list: same as above, no chord row. |
...
| E22 | Clicking (pointerdown+up with no real movement) any note — selected or not — seeks the playhead to the exact clicked point, not the note's midpoint, and dispatches no edit; the same click-down also selects it if it wasn't already. A real drag from that same pointerdown instead moves (or, on an already-selected note, resizes near an edge) it, without seeking. | Click near one edge of a wide, previously-unselected note without dragging: the note becomes selected and `currentTime()` matches the click, not the midpoint; edit list gains no new row. Click the same spot again without dragging: playhead moves again, still no new row. Drag a real distance from that first click instead: the note moves and a row appears. |
...
| E35 | **Export edits** downloads one shared filename, `sans_bass_<song>_notes_edits_<YYYY_MM_DD_HH_MM>.json` (`<song>` is `song` with no mix track loaded), timestamped at export time so a repeat export never silently overwrites the last download — not per-stem, and there is no top-level `"stem"` field any more; each stem's own data lives at `stems.<id>` instead. **Export list** is unaffected by this change in payload shape and stays per-channel, with the same `sans_bass_..._<timestamp>` filename convention and its own suffix: `sans_bass_<song>_vocals_notes_<timestamp>.html` / `sans_bass_<song>_bass_notes_<timestamp>.html` — `<title>`/`<h1>` still name which channel it came from, with the stem word always in English (`Vocals`/`Bass`) regardless of UI language, matching the existing English-only major/minor convention. | Export edits with both channels analysed: the filename matches `sans_bass_<song>_notes_edits_\d{4}(_\d{2}){4}\.json`, and the parsed JSON has `stems.vocals` and `stems.bass` keys with no top-level `stem` field. Export the 簡譜 list from the bass panel: the filename matches `sans_bass_<song>_bass_notes_\d{4}(_\d{2}){4}\.html` and its page title/`<h1>` reads `<song> — Bass — 1=<tonic> <major\|minor>` even under the 中文 UI. Export the same list again a minute later: the two downloaded filenames differ only in their timestamp suffix. |
| E36 | A well-formed edits file naming a stem the current song doesn't have loaded (e.g. a saved `guitar` entry, before guitar note detection exists) **skips just that stem and warns**, naming it — every stem the file and the song both have still imports normally. There is no back-compat path for older format versions — the old per-stem single-stem files (`{version:1, stem, edits, ...}`, from before the shared button replaced the per-panel pair), or a `version: 2` file (from before a batch's `edits` entries carried `label`/`timeLabel`) — one of those is just rejected as not a recognized edits file, same as any other malformed JSON. | Hand-craft a file with `stems.bass` and `stems.guitar` and import it into a song with only vocals/bass loaded: bass imports normally, and a warning names guitar as skipped. Import an old single-stem `<song>-bass-edits.json` file (or anything with `"version": 1` or `"version": 2`): the import-failed warning fires and nothing changes. |
...
| E38 | **⊞ Snap** (toolbar, and the `G` key with a note selected) snaps just the selected note's start and end to the current grid resolution (E37's ½/¼ toggles) as one ordinary `timeAdjust` edit — indistinguishable in the edit list from a manual resize that happened to land on grid values. Disabled without a selection or with the tempo grid off; a no-op (no new row) if the note is already exactly on-grid. | Select an off-grid note, click ⊞ (or press `G`): Start/End fields snap to the nearest grid points, one new `timeAdjust` row appears. Click ⊞ again immediately: no new row. Turn the tempo grid off: ⊞ is disabled. |
| E39 | **Snap range** snaps every note overlapping the selected range (E12/E21) to the grid in ONE batch: however many notes move, they land as a single edit-list row, and one Undo/Cmd+Z reverts all of them together — never one row per note. Disabled without a range selected or with the tempo grid off. Two notes that don't already overlap can never end up overlapping purely from this (`lib/ribbon.js`'s `snapNotesToGrid`); a note short enough for both edges to land on the same grid point gets floored to a minimum 20ms width instead of collapsing to nothing. | Select a range covering several off-grid notes, click Snap range: the edit list gains exactly ONE row regardless of how many notes moved. Press Cmd/Ctrl+Z once: every one of those notes reverts together, not one at a time. |
| E40 | **Whole song** sets the range selection to the entire song in one click — a shortcut into Snap range (E39) and Delete range (E12/E21) alike, instead of dragging a range band across the full width by hand. | With edit mode on, click Whole song: the amber range highlight covers the full zoomed pane and the full-song lane, and Snap range/Delete range both enable immediately. |
| E41 | The range-select band (E12) is also available on the **Overview** lane (the combined-stems strip above the zoomed pane), not just the zoomed pane and each stem's own lane — sharing the same selection, so a drag on any of the three surfaces updates what the other two show. Gated on an edit target being selected at all (E1), since Overview has no single stem identity to match against. | With edit mode on and a Notes chip selected, drag along the bottom of the Overview lane: the amber highlight appears there AND in the zoomed pane/stem lane at the same time range; Snap range/Delete range enable. |
| E42 | With a range selected (E12/E21) — which, per E43, means no note is selected — `Delete`/`Backspace` and `G` mirror the Delete range/Snap range buttons (E12/E39): same edits, same batching. | Drag a range with nothing else selected, press `Delete`: notes overlapping the range are removed, same as clicking Delete range. Select a different range over off-grid notes, press `G`: they snap to the grid as one batched edit, same as clicking Snap range. |
| E43 | Selecting a note and having a range selected are mutually exclusive. Clicking a note (or placing a new one via + Add note) clears any active range selection; creating or committing a range selection — dragging a range band on any surface (E12/E21/E41) or clicking Whole song (E40) — clears any selected note. At most one of the two is ever active. | Select a note, then drag a range band: the note's white outline disappears as the amber range highlight appears. With a range selected, click a note: the amber highlight disappears and the note gets its outline. Click Whole song with a note selected: the note's outline disappears and the whole-song range highlights instead. |
