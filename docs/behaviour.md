...
| E42 | With a range selected (E12/E21) and no note selected, `Delete`/`Backspace` and `G` mirror the Delete range/Snap range buttons (E12/E39) — same edits, same batching. If a note happens to be selected at the same time, the note's own Delete/`G` (E16/E38) take priority instead. | Drag a range with nothing else selected, press `Delete`: notes overlapping the range are removed, same as clicking Delete range. Select a different range over off-grid notes, press `G`: they snap to the grid as one batched edit, same as clicking Snap range. |
