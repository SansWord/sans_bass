...
| N56 | The zoomed pane's header carries a **lane selector**: one chip per stem actually loaded, plus a **Notes** chip, at the left of the seconds readout. Default selection is vocals + notes — today's fixed behaviour, unchanged unless the user picks more. | `.zoom-lane-sel .zoom-chip` count matches `tracks.filter(t => t.stem).length`; `.zoom-chip-dot.on` and `.zoom-notes-chip.active` on load. |
| N57 | Clicking a stem's dot adds or removes **its waveform** from the pane; any combination can be selected at once. | Call `toggleZoomLane('bass')` (or click the chip): `.zoom-chip-dot.on` toggles, and the canvas shows a second overlaid waveform. |
...
| N59 | With Notes off, the pitch grid, contour line and note blocks disappear entirely — there is nothing pitched to plot them against, and drawing an axis with nothing on it would be worse than no axis. | Toggle `.zoom-notes-chip`: the piano-roll shading and note-name gutter vanish from the canvas; only waveform(s) and the time ruler remain. |
| N60 | A **speaker glyph** beside each stem chip mutes/unmutes that lane exactly like clicking its row in the main lane list, and both stay in sync regardless of which one is clicked. | Click `.zoom-chip-mute` for vocals: the main `人聲`/`Vocals` row gains `.muted` (opacity `.38`) and the glyph gains `.muted` (struck through). Click the main lane row instead: the glyph updates the same way — both paths route through `applyGains()`. |
...
| N62 | The zoom lane's own label ("局部放大" / "Zoom") sits on its **own row**, above the seconds readout and chips, and is never truncated — the bug this replaced squeezed it into a 128px column shared with the seconds/buttons. | Load a song, run Find notes: `.lane.ribbon-zoom .lane-name .txt` renders its full text (`getBoundingClientRect().width` is not clipped by `text-overflow: ellipsis`, and `scrollWidth <= clientWidth` on that span). |
...
| E34 | **Export list** (`#notes-list-export`) downloads a Markdown file of the current 簡譜 reading, chunked into fixed-length blocks by note start time — a separate, human-readable export from **Export edits**' JSON round-trip. Disabled **only** while there are zero notes (via `syncJianpuControls()`) — unlike the key selectors, it does NOT need 簡譜 itself to be on: `jianpu.tonic`/`jianpu.mode` always hold a real value, either the auto-detected key (once notes exist) or the C-major default before any override, so there's always a key to export against. The "seconds per line" field (`#notes-list-secs`, default 10) sets block width. First line is an H2, `## <song> — 1=<tonic> <major\|minor>` — the song name and its trailing em dash are omitted entirely when `currentMix()` has no name (e.g. in-browser separation output). Each block is an H3, `### MM:SS - MM:SS`, followed by a line of space-separated 簡譜 tokens as its own paragraph — real Markdown headings, not a `==`-style plain-text marker, so a Markdown previewer renders the timestamp and the notes as visibly separate lines instead of merging them into one paragraph. Each token carries its own octave marks (`'` suffix per octave above the reference, `,` prefix per octave below — see N49). The major/minor word is always English, regardless of the UI language — the file is read outside the app, where the current locale doesn't apply. A note is bucketed only by its start time, never split across or duplicated between blocks even when its duration crosses a boundary. | With zero notes, `#notes-list-export.disabled` is `true`. Run **Find notes** without ever ticking 簡譜: it enables as soon as notes exist. Click it: a `<song>-notes.md` (or `song-notes.md` with no mix name) file downloads using the auto-detected key exactly as if 簡譜 had been ticked, its header line starting `## ` and each block starting `### `, the mode word reading `major`/`minor` even in the 中文 UI. Change "seconds per line" from 10 to 5 and export again: every block is half as wide, and the tokens from an old 10s block split cleanly across the two new 5s blocks with none lost or repeated. |
