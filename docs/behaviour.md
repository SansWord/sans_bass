...
| TRN-001 | Load unequal-duration synthetic stems and instrument source scheduling. | Play, pause, seek by waveform and keyboard while running/stopped, then let the longest source end. | The primary button has truthful state and bilingual accessible copy; every source shares one start time, mute never restarts it, seeks preserve running/stopped state, time labels update, and end-of-song comes from the audio graph. | `tests/transport-math.test.js`, `tests/player.test.js` | Trusted-gesture unlock and background-tab end behavior. |
