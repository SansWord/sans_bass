...
| E44 | **Export list** prints a chord guess above each bar from a chromagram over whichever guitar/piano/bass stems are loaded, independent of the channel being exported (`notes.js` mixes the stems; `lib/chords.js` drives `lib/chroma.js`). Each bar splits at its time MIDPOINT. Each half is matched, independently of detected key, to `{maj, min, 7, min7, sus2, sus4}` templates by Pearson correlation. When bass notes have been analysed and the longest-overlapping bass pitch class differs from the matched root, the label uses slash notation such as `E/G#`; otherwise it renders the bare chord label. The first label is top-left and a differing second label is centered; `second` is `null` for silence or a repeat. With none of guitar/piano/bass loaded, no chord row is rendered. | Load a song with guitar and/or piano plus bass, run **Find notes** on bass to enable slash notation, then export either notes channel: chord labels render above every bar and inversions may use slash notation. With guitar/piano loaded but bass unanalysed, labels still render without slashes. With only bass loaded, its chroma still produces labels. With no harmonic stem, no chord row appears. |
