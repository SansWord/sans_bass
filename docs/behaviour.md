...
| E35 | **Export edits** downloads one shared filename, `<song>-notes-edits.json` (or `song-notes-edits.json` with no mix track loaded) — not per-stem, and there is no top-level `"stem"` field any more; each stem's own data lives at `stems.<id>` instead. **Export list** is unaffected by this change and stays per-channel: filename and `<title>`/`<h1>` still name which channel it came from — `<song>-vocals-notes.html` / `<song>-bass-notes.html` — with the stem word always in English (`Vocals`/`Bass`) regardless of UI language, matching the existing English-only major/minor convention. | Export edits with both channels analysed: the filename ends `-notes-edits.json`, and the parsed JSON has `stems.vocals` and `stems.bass` keys with no top-level `stem` field. Export the 簡譜 list from the bass panel: the filename ends `-bass-notes.html` and its page title/`<h1>` reads `<song> — Bass — 1=<tonic> <major\|minor>` even under the 中文 UI. |
