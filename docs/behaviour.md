...
| NOTE-001 | Generate vocals-only, bass-only, both, neither, and drums-assisted fixtures; use a deterministic notes Worker except for the real-Worker smoke. | Start detection, finish channels in both orders, vary interpretation/folding/display controls, mute/show lanes, edit a zoomed chord (including a near-confidence candidate), and load another song. | Panels appear only after their channel completes; pending/running labels name exact channels and chord work remains visibly busy until complete; analysis is user-triggered and independent; interpretation reuses frames; ribbons/overview/zoom remain aligned; harmonic-stem chords appear in timed zoom intervals, ambiguous labels use a distinct color, and edits remain visible/exportable; doubtful notes stay silent. | `tests/detection-state.test.js`, `tests/notes.test.js`, `tests/pitch.test.js`, `tests/ribbon.test.js`, `tests/sonify.test.js`, `tests/chords.test.js` | Musical accuracy and final visual readability. |
