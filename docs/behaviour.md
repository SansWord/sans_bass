...
| FIXED-001 | Serve a stems zip from a cross-origin host that sends `Access-Control-Allow-Origin`, with and without `Content-Length`; also a 404, a network failure, and an ordinary player page with no declaration. | Open a page whose `<body>` carries `data-fixed-song`, watch the download, retry a failure, and follow the header's download link, follow the banner above the transport, and play far enough to see the playhead, the bar grid and an A–B region drawn. Repeat against a production build, not only the dev server. | The page never exposes a file input, drop affordance, document drag listeners, or separation panel, in dev and in the built output alike; the zip loads itself and titles the song from its folder; progress shows a percentage when the host exposes a length and bytes received when it does not; a failed download names its status and offers a retry that does not re-run a successful decode; the header offers the same zip as a download; a page with no declaration behaves exactly as the ordinary player. The page offers no note detection, note editing, chord-source or tempo controls at all, while the drums-derived tempo grid still renders over every waveform at its configured BPM and phase. It renders in Taiwanese Mandarin only, with no language toggle and no demos link, and visiting it never changes the language the main player remembers. Its embedded video reserves its box before loading and never autoplays. Its banner leads out to the campaign in a new tab, never back to itself, while the main player's banner still leads in to it. It renders light on its own warm palette, and every colour `app.js` paints into a canvas — waveform, playhead, bar grid, A–B shading and markers, and the zoomed pane — stays visible against it rather than the near-black those default to; the main player defines none of those properties and is pixel-unchanged. | `tests/fixed-song.test.js`, `tests/i18n.test.js` | Built-output check that bundling did not reorder the declaration, and one real cross-origin host for CORS and `Content-Length`. |
...
| TRN-001 | Load unequal-duration synthetic stems and instrument source scheduling. | Play, pause, seek by waveform and keyboard while running/stopped, including with the primary seek control focused, then let the longest source end. | The primary button has truthful state and bilingual accessible copy; the seek waveform exposes one bilingual slider value with current/total time; every source shares one start time, mute never restarts it, seeks preserve running/stopped state, time labels update, and end-of-song comes from the audio graph. | `tests/transport-math.test.js`, `tests/player.test.js` | Trusted-gesture unlock and background-tab end behavior. |
...
| NOTE-001 | Generate vocals-only, bass-only, both, neither, and drums-assisted fixtures; include bars with agreeing halves, a real mid-bar change, one weak half, and two weak halves; use a deterministic notes Worker except for the real-Worker smoke. | Start detection, finish channels in both orders, vary interpretation/folding/display controls, mute/show lanes, choose a capo fret, toggle the vocals/other chord-source extras, edit a zoomed chord (including a near-confidence candidate), and load another song. | Panels appear only after their channel completes; pending/running labels name exact channels and chord work remains visibly busy until complete; chord analysis waits for every running note channel so it uses vocal key and bass inversions in one complete pass; toggling a vocals/other chord-source extra re-derives chords in place without discarding existing corrections, and resets to both-off on a new song; analysis is user-triggered and independent; interpretation reuses frames; ribbons/overview/zoom remain aligned; agreeing strong halves collapse to one full-bar chord, a strong full bar carries one strong half across a weak partner, distinct strong halves and inversion changes remain separate, fully weak bars stay blank, capo transposes play shapes and play key without changing concert pitch, ambiguous labels use a distinct color, and edits remain visible/exportable; doubtful notes stay silent. | `tests/detection-state.test.js`, `tests/notes.test.js`, `tests/pitch.test.js`, `tests/ribbon.test.js`, `tests/sonify.test.js`, `tests/chords.test.js` | Musical accuracy and final visual readability. |
| TEMPO-001 | Load drums click tracks with known BPM/phase plus vocals/bass, including an excluded intro. | Detect, override/halve/double/nudge meter, enable subdivisions, choose/clear a range, re-detect, export/import, and load another song. | Controls gate on drums evidence; the grid renders from a tempo detected on its own, with no channel analysed, and is pixel-identical once notes do arrive and a ribbon takes over as its source; toggling the grid, phase or meter repaints without one; shared grids align across canvases; range and manual state behave predictably; resets and exported state are exact. | `tests/tempo.test.js`, `tests/ribbon.test.js`, `tests/notes-edits.test.js` | Auditory/metronome judgment for ambiguous real music. |
| EDIT-001 | Supply controlled notes with overlapping time/pitch and exact duplicates, plus an active tempo grid and corrected chords. | Select/click/drag/resize/add/delete/split/repitch/nudge/snap notes; select ranges on lane/overview/zoom; edit chords; use Whole song, keyboard commands, batches, undo, and import/export. | Note and range targets are mutually exclusive; hit selection respects pitch/topmost order; clicks seek while drags edit; commands act on the outline; batches undo once in reverse order; note fields, chord corrections, chord-source extras, and persistence round-trip. | `tests/editor-state.test.js`, `tests/notes-edits.test.js`, `tests/pitch.test.js`, `tests/ribbon.test.js`, `tests/time.test.js` | Final canvas interaction/colour judgment. |
...
| LANG-001 | Start clean, then with stored and blocked storage; load a song and establish routing, loop, status, separation, notes, and tempo UI. | Switch English/Taiwanese Mandarin during playback. | Stored choice wins and reaches React-rendered controls as well as `<html lang>` and the tab title, whichever order the bundler evaluates boot in; first visit is not persisted; blocked storage cannot stop boot; all visible copy rerenders while stable IDs, routing, transport, canvases, and audio remain unchanged. | `tests/i18n.test.js`, `tests/demo-header.test.jsx`, `tests/player.test.js` | Audibly confirm no gap during a trusted playback run. |
...
Switch the list between English and Taiwanese Mandarin; verify the heading, description,
