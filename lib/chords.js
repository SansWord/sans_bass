...
export function detectChordTimeline(harmonicSamples, sampleRate, barBounds, bassNotes, key = null) {
  ...
  return halves.map((half, i) => ({
    start: half.halfStart,
    end: half.halfEnd,
    barStart: i % 2 === 0,
    label: labelForHalf(decoded[i], half.notesInWindow, half.halfStart, half.halfEnd),
    candidates: visibleCandidates(half.candidates, half.notesInWindow, half.halfStart, half.halfEnd),
  }));
}
...
export function detectChords(harmonicSamples, sampleRate, barBounds, bassNotes, key = null) {
  ...
  const result = [];
  for (let i = 0; i < bounds.length - 1; i++) {
    const first = timeline[i * 2].label;
    const second = timeline[i * 2 + 1].label;
