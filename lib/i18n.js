 ...
 *
 * ESM, no window bridge — app.js, separate.js and notes.js all import it directly, and a
 * module is a singleton, so all three share exactly one dictionary. */
...
export const DICT = {
  'zh-TW': {
    ...
    'nav.demos': '匯出簡譜範例',
    'nav.player': '播放器',
    'demos.title': '匯出簡譜範例 — sans_bass',
    'demos.description': '瀏覽分享的匯出簡譜與 HTML 範例。',
    'demos.back': '← 返回 sans_bass 播放器',
    'demos.count': '範例數量：{count}',
    'demos.empty': '尚未發布範例。',
    ...
    // 固定曲目頁（puma_taipei_smooth.html）：這頁沒有載入按鈕，歌曲由頁面自己下載。
    // 歌名是專有名詞，兩個語系一字不差 —— 跟 stem id、檔名同一條規則。
    'puma.title': '《台北順起來 𝙁𝙪𝙩𝙪𝙧𝙚 𝙎𝙤𝙤𝙣》— 分軌練習',
    'fixed.loading': '準備歌曲中…',
    'fixed.preparing': '即將開始…',
    'fixed.progressPercent': '下載分軌中 — {percent}%（共 {mb} MB）',
    'fixed.progressBytes': '下載分軌中 — 已下載 {mb} MB',
    'fixed.privacy': '分軌在你的瀏覽器本機解碼，不會上傳任何東西。',
    'fixed.failed': '無法下載分軌',
    'fixed.retry': '重試',
    'fixed.download': '下載分軌.zip',
    ...
    'play.aria': '播放／暫停',
    'seek.aria': '搜尋播放位置',
    'seek.value': '{current}／{duration} · {speed}',
    'ctl.play': '播放',
    'ctl.volume': '音量',
    'ctl.speed': '速度',
    'lane.tip': '點擊以靜音或取消靜音這條軌道',
    'hint.click': '點擊軌道左側的<strong>名稱區塊</strong>，即可靜音或取消靜音該軌',
    'hint.keys': '空白鍵播放 · ←→ 前後 5 秒 · 1-6 選軌 · 0 全部靜音／取消靜音 · <strong>a</strong>/<strong>b</strong> 循環 · <strong>c</strong> 清除 A–B 循環 · <strong>[</strong>/<strong>]</strong> 調整速度 ±5%（<strong>Shift</strong> ±1%） · <strong>\\</strong> 重設速度',
    ...
    'status.commandFailed': '播放器指令失敗：{message}',
    ...
    'notes.overview': '總覽',
    'notes.overviewTip': '點擊以跳轉至歌曲中的位置',
    'notes.zoom': '局部放大',
    'notes.zoomTip': '點擊以跳轉，拖曳以平移，滾輪縮放',
    'notes.zoomIn': '放大',
    'notes.zoomOut': '縮小',
    'notes.zoomHalfBeatTip': '顯示／隱藏每拍二分之一處的虛線',
    'notes.zoomQuarterBeatTip': '顯示／隱藏每拍四分之一處的虛線',
    'notes.zoomLaneShowTip': '{lane}：在放大窗格中顯示／隱藏波形',
    'notes.zoomLaneMuteTip': '{lane}：靜音／取消靜音',
    ...
    'notes.chord': '和弦',
    'notes.capo': '移調夾',
    'notes.capoTip': '移調夾品數；顯示的和弦與演奏調會向下移調',
    'notes.playKey': '演奏調 {key}',
    'notes.chordEditTip': '編輯目前和弦；相近信心的候選會顯示在建議清單中',
    'notes.chordRedetect': '重新偵測和弦',
    'notes.chordRedetectTip': '使用目前的 BPM、相位與拍號重新偵測和弦',
    'notes.chordDetecting': '正在偵測和弦…',
    'notes.chordWaiting': '正在等待音符偵測完成…',
    'notes.chordCandidates': '其他候選',
    'notes.chordCandidatesTip': '顯示所有信心相近的和弦候選',
    'notes.hide': '隱藏音符',
    'notes.edit': '編輯音符',
    ...
    'notes.editSnap': '對齊拍點',
    'notes.editSnapTip': '將選取的音符對齊拍點格線',
    'notes.editRangeSnap': '對齊範圍',
    'notes.editRangeSnapTip': '將選取範圍內的所有音符對齊拍點格線',
    'notes.editWholeSong': '整首歌',
    'notes.editWholeSongTip': '選取整首歌作為範圍',
    ...
    'notes.editSnapLabel': '對齊拍點',
    'notes.export': '匯出編輯',
    'notes.import': '匯入編輯',
    'notes.importFailed': '匯入失敗：{message}',
    'notes.importMismatch': '這個檔案看起來是給「{song}」用的，與目前載入的歌曲不同。',
    'notes.importStemSkipped': '這個檔案也包含「{stems}」的編輯，但這首歌沒有載入這些音軌，已略過。',
    'notes.listBars': '每行小節數',
  ...
  'en': {
    ...
    'nav.demos': 'Demos',
    'nav.player': 'Player',
    'demos.title': 'Demos — sans_bass',
    'demos.description': 'Explore shared notation exports and HTML demos.',
    'demos.back': '← Back to sans_bass player',
    'demos.count': 'Demos: {count}',
    'demos.empty': 'No demos published yet.',
    ...
    // Fixed-song page (puma_taipei_smooth.html): no load button here — the page fetches its own song.
    // The record's name is a proper noun and is byte-identical in both locales, the same
    // rule that keeps stem ids and filenames untranslated. Only the words around it move.
    'puma.title': '《台北順起來 𝙁𝙪𝙩𝙪𝙧𝙚 𝙎𝙤𝙤𝙣》— stem practice',
    'fixed.loading': 'Getting the song ready…',
    'fixed.preparing': 'Starting…',
    'fixed.progressPercent': 'Downloading stems — {percent}% of {mb} MB',
    'fixed.progressBytes': 'Downloading stems — {mb} MB so far',
    'fixed.privacy': 'The stems are decoded locally in your browser. Nothing is uploaded.',
    'fixed.failed': 'Could not download the stems',
    'fixed.retry': 'Try again',
    'fixed.download': 'Download stems .zip',
    ...
    'seek.aria': 'Seek',
    'seek.value': '{current} of {duration} · {speed}',
    'ctl.play': 'Play',
    'ctl.volume': 'Volume',
    'ctl.speed': 'Speed',
    ...
    'hint.keys': 'space play · ←→ seek 5s · 1-6 stem · 0 mute/unmute all · <strong>a</strong>/<strong>b</strong> loop · <strong>c</strong> clear the A–B loop · <strong>[</strong>/<strong>]</strong> speed ±5% (<strong>Shift</strong> ±1%) · <strong>\\</strong> reset speed',
    ...
    'status.commandFailed': 'Player command failed: {message}',
    ...
    'notes.detecting': 'Detecting: {stems}…',
    ...
    'notes.overviewTip': 'Click to jump to a point in the song',
    'notes.zoom': 'Zoom',
    'notes.zoomTip': 'Click to seek, drag to pan, scroll to zoom',
    'notes.zoomIn': 'Zoom in',
    'notes.zoomOut': 'Zoom out',
    'notes.zoomHalfBeatTip': 'Show/hide dotted lines at the half beat',
    'notes.zoomQuarterBeatTip': 'Show/hide dotted lines at the quarter beat',
    'notes.zoomLaneShowTip': '{lane}: show/hide its waveform in the zoomed pane',
    'notes.zoomLaneMuteTip': '{lane}: mute/unmute',
    ...
    'notes.chord': 'Chord',
    'notes.capo': 'Capo',
    'notes.capoTip': 'Capo fret; displayed chords and play key transpose down',
    'notes.playKey': 'Play key {key}',
    'notes.chordEditTip': 'Edit the current chord; similarly confident candidates appear as suggestions',
    'notes.chordRedetect': 'Re-detect chords',
    'notes.chordRedetectTip': 'Detect chords again using the current BPM, phase, and meter',
    'notes.chordDetecting': 'Detecting chords…',
    'notes.chordWaiting': 'Waiting for note detection…',
    'notes.chordCandidates': 'Other candidates',
    'notes.chordCandidatesTip': 'Show every similarly confident chord candidate',
    ...
    'notes.editSnap': 'Snap',
    'notes.editSnapTip': 'Snap the selected note to the beat grid',
    'notes.editRangeSnap': 'Snap range',
    'notes.editRangeSnapTip': 'Snap every note inside the selected range to the beat grid',
    'notes.editWholeSong': 'Whole song',
    'notes.editWholeSongTip': 'Select the whole song as the range',
    ...
    'notes.editSnapLabel': 'Snap to grid',
    ...
    'notes.importStemSkipped': 'This file also has edits for {stems} — not loaded in this song, so skipped.',
    'notes.listBars': 'Bars per line',
...
// Which dictionary key names the tab. Every entry page but a fixed-song one is the player.
let titleKey = 'app.title';
...
/* zh-TW covers Taiwanese Mandarin; Simplified-script tags go to English, because the whole
 * point of the zh-TW copy is Taiwan terminology, which does not serve zh-CN readers. */
function isTaiwanLocale(tag) {
...
export function detectLocale(langs) {
  const list = langs ||
    (window.navigator && navigator.languages) ||
    (window.navigator && navigator.language ? [navigator.language] : []) ||
    [];
  if (!list.length) return DEFAULT_LOCALE;
  for (const tag of list) if (isTaiwanLocale(tag)) return 'zh-TW';
  return 'en';
}
...
export function setLocale(loc, opts) {
  ...
  if (persist) {
    try { window.localStorage.setItem(STORAGE_KEY, locale); } catch (e) { /* see above */ }
  }
  if (window.document) {
    document.documentElement.lang = locale;
    // Only rewrite the tab title for a document i18n actually booted. tests/i18n.test.js
    // imports this file (under jsdom) to poke at the dictionary and must keep its own title.
    if (booted) document.title = t(titleKey);
    apply(document);
  }
  window.dispatchEvent(new CustomEvent('sansbass:langchange', { detail: { locale } }));
}
...
/**
 * Boot. Called from a one-line script in an entry page's <head>, so `lang` and the tab
 * title are right before anything paints; the DOM walk waits for the body to exist.
 *
 * `opts.titleKey` names the dictionary key the tab title tracks, for an entry page that is
 * not the player itself — it stays right across a language switch, which a literal <title>
 * in the markup would not.
 *
 * Not run on import: tests/i18n.test.js imports this file (under jsdom) to poke at the
 * dictionary and must not have its own <title> rewritten or its markup walked.
 */
export function init(opts) {
  booted = true;
  if (opts && opts.titleKey) titleKey = opts.titleKey;
  ...
  /* Announce it, exactly as setLocale does, because this may not have run first.
   *
   * Whether this module's init() runs before or after app.js mounts the React shell is a
   * bundler's decision, not this file's: a page's module scripts are merged into one entry
   * chunk and static imports hoist, so app.js's body can run before the <head> call that
   * chose the locale. While app.js was inlined into the entry chunk this happened to come
   * first; the moment a second entry page shared app.js, Rollup split it out and the order
   * inverted. Subscribers that mounted against the pre-init default (React reads getLocale()
   * once at mount) then never heard about the real choice, leaving a page whose <html lang>
   * and title were English and whose React-rendered controls were Taiwanese Mandarin.
   *
   * Publishing here converges either order. When init() does run first there are no
   * subscribers yet and this is a no-op. */
  window.dispatchEvent(new CustomEvent('sansbass:langchange', { detail: { locale } }));
}
