...
function capoValues(label) {
  return Array.from({ length: 12 }, (_, fret) => transposeChordLabel(label, -fret));
}

function capoSpan(className, values, capo) {
  return `<span class="${className}" data-capo-values="${escapeHtml(JSON.stringify(values))}">${escapeHtml(values[capo])}</span>`;
}
...
function chordsHtml(pair, capo, interactive) {
  const first = pair && pair.first ? (interactive ? capoSpan('chord-first', capoValues(pair.first), capo) : `<span class="chord-first">${escapeHtml(pair.first)}</span>`) : '';
  const second = pair && pair.second ? (interactive ? capoSpan('chord-second', capoValues(pair.second), capo) : `<span class="chord-second">${escapeHtml(pair.second)}</span>`) : '';
 ...
 *  in concert pitch, computed from whichever guitar/piano/bass stems are loaded, regardless
 *  of the channel being exported. `tonic` enables the capo selector; `capo` is its initial fret. When omitted entirely (none of those stems is loaded), no `.chords`
 *  element is rendered and `.bar` keeps its previous visual height/appearance. */
export function jianpuHtml({ title, bars, barsPerLine, bpm, beatsPerBar, chords, tonic, capo = 0 }) {
  const interactive = Number.isInteger(tonic);
  const keys = interactive ? capoValues(['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'][tonic])
    .map((key) => tr('notes.playKey', { key })) : [];
  const lines = [];
  for (let i = 0; i < bars.length; i += barsPerLine) {
    const cells = bars.slice(i, i + barsPerLine)
      .map((frags, j) => {
        const chordRow = chords ? chordsHtml(chords[i + j], capo, interactive) : '';
...
${interactive ? `<div class="capo-controls"><label>${escapeHtml(tr('notes.capo'))} <select id="capo">${Array.from({ length: 12 }, (_, fret) => `<option value="${fret}"${fret === capo ? ' selected' : ''}>${fret}</option>`).join('')}</select></label> &nbsp; ${capoSpan('play-key', keys, capo)}</div>` : ''}
<p class="tempo">♩ = ${bpm.toFixed(1)} &nbsp;&nbsp; ${beatsPerBar}/4</p>
${lines.join('\n')}
${interactive ? `<script>
const capo = document.getElementById('capo');
function updateCapo() {
  document.querySelectorAll('[data-capo-values]').forEach((element) => {
    element.textContent = JSON.parse(element.dataset.capoValues)[Number(capo.value)];
  });
}
capo.addEventListener('change', updateCapo);
updateCapo();
</script>` : ''}
</body></html>
`;
