 ...
 *
 * `barBounds` is rounded through roundSeconds (same millisecond precision every note's own
 * start/end is stored at, lib/time.js) before use. lib/ribbon.js's beatTimes() — the usual
 * source of these boundaries — builds them by accumulating `+= periodSec` in a loop, an
 * unrounded float; a note snapped onto the SAME nominal grid point instead goes through
 * snapToGrid()'s direct multiplication and then roundSeconds. The two paths can disagree by
 * a fraction of a millisecond, and comparing a rounded note time against an unrounded
 * boundary put a sub-millisecond sliver of a boundary-landing note in the wrong bar — visible
 * as a spurious tied grace note, since noteRhythm floors any nonzero duration up to a full
 * displayed unit. Rounding both sides to the same precision they're already stored at
 * collapses that drift to zero.
 *
...
function layoutBars(notes, barBounds, tonicPc, mode, refOctaveIndex, beatSec) {
  ...
  const bars = [];
  for (let i = 0; i < bounds.length - 1; i++) bars.push([]);
  ...
  for (const note of sorted) {
    ...
    for (let i = 0; i < bounds.length - 1; i++) {
      ...
      if (start >= barEnd || note.end <= bounds[i]) continue;
