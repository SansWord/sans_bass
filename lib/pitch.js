 ...
 * 41.2 Hz, versus the vocal window's 80 Hz floor. This range was widened twice, each time
 * measured on real stems rather than picked from arithmetic alone — see
 * docs/transcription.md for both sweeps and docs/tuning-cases.md for the reasoning:
 *
 * 1. tauMax 269 -> 300 (D1, 36.7 Hz): a band tuned down a half step (Eb standard) puts its
 *    lowest string at D#1 (38.9 Hz), below the original E1-only floor. tauMax 269 missed
 *    that note on "6 南國的風"'s real bass stem entirely (locked onto a boundary-clamped
 *    ~41 Hz reading, ~90 cents off); 300 resolves it as one clean note, and going further
 *    (320) bought nothing more.
 *
 * 2. tauMax 300 -> 379, window 1024 -> 1408 (B0 + 1 semitone margin, 29.1 Hz): headroom for
 *    a 5-string bass's low B (30.9 Hz standard), chosen ahead of a confirmed case rather
 *    than after one, on the basis of cost alone plus the periods-per-window ratio — no
 *    5-string stem exists in this project to validate against yet. Widening `window`
 *    alongside `tauMax` keeps that ratio near ~3.7 (matching the vocal range's own), rather
 *    than letting it drop toward 512/269's ~1.9 — the ratio the original window sweep
 *    identified as the noisiest tried. This IS a measured, accepted accuracy cost, not a
 *    free extension: re-run on the same two tracks used to validate step 1, the wider
 *    settings raise octave-outlier time share (2.8%->3.4% on "6 南國的風", 6.9%->7.2% on
 *    "9 繼續向前行") and shift the duration-weighted median note down a semitone on the
 *    second track (B1->A#1) — a small systematic pull toward lower readings, not just edge
 *    noise. The D#1 case from step 1 does not regress under the wider settings. Decode cost
 *    is roughly double 300/1024's (measured +88.6%), still small in absolute terms (~1.1 s
 *    warm, ~7 s cold becomes ~2 s / ~13 s) but real. Accepted deliberately in favour of
 *    future-proofing for a 5-string track; if that trade reads wrong for a future song, the
 *    fallback is a per-song override, not reverting the shared default. */
export const BASS_RANGE = { tauMin: 27, tauMax: 379, window: 1408 };
 ...
 * language) — also the source components/DetectionPanel.jsx uses for the key tonic select. */
export const PITCH_CLASS_NAMES = NOTE_NAMES;
...
/* These names share one flat options bag with YIN_DEFAULTS, TRACK_DEFAULTS,
 * SEGMENT_DEFAULTS and HMM_DEFAULTS — interpret() passes the same `params` object to the
 * segmenter and to foldOctaves, each ignoring what it does not read. The sets are disjoint
 * today; a new knob here must not reuse a name from any of them. */
export const FOLD_DEFAULTS = {
  ...
  confidentWithin: 1.5,  // semitones; see foldOctaves for why this is not a "tune me" knob
};
 ...
 *
 *   { from, state: 'folded', shift }   corrected: midi/cents/name are updated, `from` is
 *                                      the detector's original midi
 *   { from, state: 'doubt', doubt: true }   untrusted: midi is untouched, and it should not
 *                                           be sounded
 *
 * Read `fix.state` ('folded' | 'doubt') to discriminate. Do not test `fix.shift` for
 * truthiness: it is never 0 today only because a zero shift is routed to doubt.
 *
...
export function foldOctaves(notes, opts = {}) {
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    if (!context.length) { n.fix = { from: n.midi, state: 'doubt', doubt: true }; continue; }
    ...
    /* The threshold is the only thing keeping an octave-plus-a-fifth error out of the
     * folded bucket, so it is not a free tuning knob.
     *
     * A power-of-two harmonic error (2nd/4th/8th) leaves a residual of exactly 0 after the
     * right octave shift. A 3rd- or 6th-harmonic error leaves 4.98 — which is why the
     * original 5 was wrong: it sat ON the failure mode rather than between the populations.
     * Melodic movement blurs both, so measured on real material they overlap (foldable
     * 0-3.5, unfoldable 1.5-7) and NO threshold separates them cleanly. 1.5 was chosen to
     * fold nothing it cannot justify: on ng_kipin it admits no odd-harmonic error whose
     * residual it can see, at the cost of about half the true corrections, which become
     * doubtful instead. It bounds the RESIDUAL, not the error: when the neighbours
     * themselves sit a fifth from the truth, a 6th-harmonic error lands on them with
     * residual 0 and no threshold can catch it. That is inherent to a neighbour-only method.
     * Raising it trades that guarantee for coverage. Measure before you touch it. */
    if (bestK === 0 || bestD > confidentWithin) {
      n.fix = { from: n.midi, state: 'doubt', doubt: true };
      continue;
    }
    const midi = n.midi + 12 * bestK;
    n.fix = { from: n.midi, state: 'folded', shift: bestK };
    n.midi = midi;
    n.cents = +(n.cents + 1200 * bestK).toFixed(1);   // shift the MEASURED pitch, don't re-quantise
    n.name = noteName(midi);
  }
  return out;
}

/** Counts of folded/muted (doubtful) notes, read from each note's own `fix.state` — the
 *  same contract foldOctaves's doc comment above documents. Lifted out of notes.js's
 *  syncFoldControls() (Phase 6a) so the derivation is Node-testable and shared with the
 *  React-owned interpretation panel. */
 ...
 *
 * Every edit that touches an existing note (`octave`, `pitchNudge`, `timeAdjust`) sets
 * `fix = { state: 'manual', from }`, where `from` is the EARLIEST known original pitch —
 * carried forward from a prior `fix.from` if there is one, so a note that was folded and then
 * hand-corrected still remembers what the detector first said. This both satisfies the
 * existing rule that a `doubt` note must have its `fix` cleared or replaced (`lib/sonify.js`
 * skips it otherwise) and gives every manually-touched note a colour distinct from
 * plain/folded/doubt — including a pure time-adjust, which changes no pitch but is still a
 * human override of what the detector produced. `add`ed notes get `fix = { state: 'manual' }`
...
export function applyEdits(notes, edits) {
  ...
  for (const e of edits) {
    if (e.type === 'add') {
      list.push({
        start: roundSeconds(e.start),
        end: roundSeconds(e.end),
        midi: e.midi,
        cents: e.midi * 100,
    ...
    if (e.type === 'delete') {
      ...
      list[idx] = {
        ...n,
        start: roundSeconds(n.start + (e.dStart || 0)),
        end: roundSeconds(n.end + (e.dEnd || 0)),
        fix: { state: 'manual', from },
      };
    }
  }
