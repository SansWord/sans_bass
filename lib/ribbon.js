...
/**
 * Nearest grid time to `t`, at 1/`divisionsPerBeat` resolution (1 = beat only, 2 = half,
 * 4 = quarter — same meaning as subdivisionTimes' argument). Shares beatTimes/subdivisionTimes'
 * phase-normalisation, but answers a single point in O(1) rather than walking the whole song —
 * needed because a note drag calls this on every pointermove. Returns `t` unchanged when there
 * is no usable tempo.
 */
function snapToGrid(tempo, t, divisionsPerBeat) {
  if (!tempo || !tempo.bpmValue || tempo.bpmValue <= 0) return t;
  ...
  const periodSec = periodMs / 1000;
  const firstT = phase / 1000;
  const step = periodSec / Math.max(1, divisionsPerBeat || 1);

  const k = Math.round((t - firstT) / step);
  return firstT + k * step;
}

const SNAP_MIN_DUR = 0.02;   // the same shortest-editable-note floor app.js's drag-resize uses

/**
 * One note's snapped [newStart, newEnd] at `1/divisionsPerBeat` grid resolution — the single
 * primitive both snapNotesToGrid (below) and app.js's editSnapNote build on, so the floor and
 * rounding rules below can't drift out of sync between the single-note and batch snap paths
 * the way they once did (a note short enough for both edges to round to the SAME grid point
 * collapsed to zero width when only one of the two callers applied the floor — a zero-width
 * note can never be found again by ANY anchor search, `start <= at < end` being false for
 * every `at` when start === end, which orphaned the next edit that tried to touch it).
 *
 * Both edges round to the NEAREST grid point, and nearest-rounding is monotonic non-
 * decreasing (t1 <= t2 implies snapToGrid(t1) <= snapToGrid(t2) for any fixed tempo/
 * division) — which is what makes snapNotesToGrid's neighbour-overlap guarantee hold with no
 * clamping needed. The one case nearest-rounding can't fix on its own is a single note whose
 * own two edges land on the SAME point; the floor below extends its end instead.
 *
 * Both results go through roundSeconds (lib/time.js) — the same millisecond precision
 * `interpret()` and `applyEdits()` already store every note position at (lib/pitch.js).
 * Matching it here matters beyond cosmetics: `applyEdits` reconstructs a replayed note as
 * `note.start + edit.dStart` and rounds THAT sum again, so an edit built from a
 * full-precision delta against an already-rounded note can replay a fraction of a
 * millisecond off from what this function computed — enough, in rare cases, to land outside
 * a neighbouring note's boundary and orphan an unrelated edit. Rounding here keeps a stored
 * delta exactly reproducible under that same reconstruction.
 */
function snapNoteEdges(tempo, start, end, divisionsPerBeat) {
  const newStart = roundSeconds(snapToGrid(tempo, start, divisionsPerBeat));
  let newEnd = roundSeconds(snapToGrid(tempo, end, divisionsPerBeat));
  if (newEnd - newStart < SNAP_MIN_DUR) newEnd = roundSeconds(newStart + SNAP_MIN_DUR);
  return { newStart, newEnd };
}

/**
 * Snapped positions for a batch grid-snap: every note in `notes` whose span overlaps
 * `[range.from, range.to)` — or every note, when `range` is null — gets both edges snapped
 * via snapNoteEdges above.
 *
 * No neighbour-clamping is needed to avoid overlap: two notes that don't already overlap can
 * never end up overlapping purely from snapping their edges independently — see
 * snapNoteEdges' comment on why nearest-rounding is monotonic.
 *
 * Returns only the notes that actually moved, as `[{ note, newStart, newEnd }, …]` — a note
 * already exactly on-grid is omitted so a caller doesn't build a no-op edit for it.
 */
function snapNotesToGrid(notes, tempo, divisionsPerBeat, range) {
  const out = [];
  if (!tempo || !tempo.bpmValue || tempo.bpmValue <= 0 || !notes) return out;

  for (const note of notes) {
    if (range && !(note.start < range.to && note.end > range.from)) continue;
    const { newStart, newEnd } = snapNoteEdges(tempo, note.start, note.end, divisionsPerBeat);
    if (newStart === note.start && newEnd === note.end) continue;
    out.push({ note, newStart, newEnd });
  }
  return out;
}

export { pitchRange, contourSegments, contourColumns, zoomPeaks, zoomWindow, beatTimes, subdivisionTimes, snapToGrid, snapNoteEdges, snapNotesToGrid };
