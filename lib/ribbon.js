 ...
 *
 * Both results are rounded to 4 decimal places (0.1 ms) — the same precision `interpret()`
 * and `applyEdits()` already store every note position at (lib/pitch.js). Matching it here
 * matters beyond cosmetics: `applyEdits` reconstructs a replayed note as
 * `note.start + edit.dStart` and rounds THAT sum to 4dp again, so an edit built from a
 * full-precision delta against an already-4dp note can replay a few tenths of a millisecond
 * off from what this function computed — enough, in rare cases, to land outside a
 * neighbouring note's boundary and orphan an unrelated edit. Rounding here keeps a stored
 * delta exactly reproducible under that same reconstruction.
 */
function snapNoteEdges(tempo, start, end, divisionsPerBeat) {
  const newStart = +snapToGrid(tempo, start, divisionsPerBeat).toFixed(4);
  let newEnd = +snapToGrid(tempo, end, divisionsPerBeat).toFixed(4);
  if (newEnd - newStart < SNAP_MIN_DUR) newEnd = +(newStart + SNAP_MIN_DUR).toFixed(4);
