...
export function scheduleNotes(ctx, destination, notes, opts = {}) {
  ...
  /* pump() below walks its event queue once and stops the moment it sees something past
   * the horizon, trusting that everything after is later still. That only holds if `notes`
   * is chronologically ordered — true of interpret()'s output, false the instant
   * lib/pitch.js's applyEdits() appends a hand-added or split-off note to the end of the
   * list regardless of its own start time. Sorting a copy here, once, keeps that trust
   * correct without asking every other piece of code to maintain an order it has no other
