...
function resetRatePercent() {
  return RATE_DEFAULT;
}

function rateChangePlan(fromPercent, toPercent) {
  const from = clampRatePercent(fromPercent);
  const to = clampRatePercent(toPercent);
  if (from === to) return 'none';
  return (from === RATE_DEFAULT) !== (to === RATE_DEFAULT) ? 'rebuild' : 'update';
}
...
export { RATE_MIN, RATE_MAX, RATE_STEP, RATE_FINE_STEP, RATE_DEFAULT,
         clampRatePercent, nudgeRatePercent, resetRatePercent, rateChangePlan,
         currentTimeAtRate };
