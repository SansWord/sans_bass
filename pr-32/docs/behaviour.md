...
| N3 | While analysing: the shared **Find notes** button (`#notes-go-all`) disabled, one status message, no progress bar. `f0Track` reports no progress, and a bar that jumps 0 → 100 is worse than none. | `#notes-go-all.disabled`, `#status.textContent`. |
...
| N22 | One shared **Find notes** button (`#notes-go-all`), not one per panel: clicking it runs analysis on every channel that still needs it (has a stem, no notes yet) and leaves the rest alone. Once notes are found in a panel, that panel's own show/hide toggle appears — the same swap the separation panel does with Separate → Save. The shared button itself disables only once **every** melodic stem present has been analysed (N22a); with one of two stems still pending, it stays enabled so that stem can still be triggered. Loading a new song re-enables it and hides both toggles again. | Computed `display` of `#notes-show-vocals` / `#notes-show-bass`; `#notes-go-all.disabled` after analysing only one of two loaded stems (still `false`) vs. after both (`true`). |
| N22a | The shared button reflects what the loaded song actually has: a zip with only a bass stem (no vocals) leaves it enabled, and clicking it analyses bass only — `#notes-vocals` stays hidden throughout, per its own stem-presence rule (N1), and is never touched. A zip with **neither** melodic stem disables the button immediately on load, with no click needed to observe it — disabled, not hidden, so its state stays legible even when there's nothing to detect. | Load a bass-only zip: `#notes-go-all.disabled` is `false` and `#notes-vocals.hidden` is `true`; click it and only `#notes-bass`'s state changes (`#notes-tune-bass` shown, `#notes-count-bass` populated). Load a zip with neither vocals nor bass (e.g. drums+guitar+piano+other only): `#notes-go-all.disabled` is `true` right after `buildUI()`, before any click. |
