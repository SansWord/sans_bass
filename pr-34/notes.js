...
import { interpret, applyEdits, detectKey, notesToChroma, relativeKey, stemMismatch, BASS_RANGE }
  from './lib/pitch.js?v=1.18.7';
import { scheduleNotes } from './lib/sonify.js?v=1.18.7';
...
tempoEl.redetect.addEventListener('click', () => {
  ...
  if (!drums) return;
  const w = new Worker('./notes.worker.js?v=1.18.7', { type: 'module' });
...
function createNotesChannel(stem, els) {
  ...
  function analyse() {
    ...
    worker = new Worker('./notes.worker.js?v=1.18.7', { type: 'module' });
    worker.onmessage = (e) => {
      const m = e.data;
      worker.terminate();
      worker = null;
