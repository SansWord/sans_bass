...
test('i18n: every key used in index.html exists in both locales', async () => {
  ...
  // recently Phase 6c's edit-list/list-export row, which dropped this from 5 (Phase 6b's
  // `#notes-tempo` panel had dropped it from 15). Still high enough to catch an accidental
  // wholesale strip of the remaining legacy markup's annotations.
  assert(keys.size >= 4, `only found ${keys.size} annotated keys — did the markup change?`);
  const missing = [];
  for (const k of keys) {
    for (const loc of I18N.LOCALES) {
...
test('i18n: translating labels never renames a stem', () => {
  ...
  assertEq(SansStems.STEMS.bass.label, 'Bass',
  ...
  const out = SansStems.assignStems(ids.map((s) => ({ name: `${s}.wav` })));
  ...
  for (const name of ['貝斯.wav', '人聲.wav', '鼓組.wav']) {
    assertEq(SansStems.detectStem(name), null,
